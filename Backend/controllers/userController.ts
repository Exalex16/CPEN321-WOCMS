...
export class userController {
    ...
    async postuser(req: Request, res: Response, nextFunction: NextFunction) {
        ...
        try {
            const { googleId, googleName } = req.body;
            ...
            if (existingUser) {
                ...
                return res.status(200).send({ message: "User profile updated", googleId });
            } else {
                // Create new user
                const newUser = {
                    googleId,
     ...
     */
     async superviseAction(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const result = await db.collection("users").updateOne({ googleId }, { $set: update });
    ...
    async deleteUser(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const db = clinet.db("User");
            const result = await db.collection("users").deleteOne({ googleId });
